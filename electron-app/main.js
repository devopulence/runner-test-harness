...
function loadEnvironments() {
  ...
  // Discover all environment directories (aws-ecs, openshift-sandbox, etc.)
  const envDirs = fs.readdirSync(TEST_RESULTS_DIR, { withFileTypes: true })
    .filter(d => d.isDirectory())
    .map(d => d.name);

  for (const envName of envDirs) {
    const envPath = path.join(TEST_RESULTS_DIR, envName);
    const results = {};

    const testDirs = fs.readdirSync(envPath, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => d.name);
    ...
    for (const dir of testDirs) {
      ...
      const data = { testType, flavor, dirName: dir, environment: envName };
